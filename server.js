...
app.get('/', (req, res) => {
    res.send('🔥 Servidor do RPG VR/PC está VIVO e com AI Inteligente!');
});
...
db.ref('cenario_inimigos').on('child_added', snap => {
    ...
    estadoInimigos[id] = {
        ...
        range: data.range || stats.range, // Agora lê o range dinâmico do Admin!
        ...
        ultimoAvistamento: Date.now() // NOVO: Relógio de perseguição
    };
...
db.ref('cenario_inimigos').on('child_changed', snap => {
    ...
    if (estadoInimigos[id] && data) {
        ...
        // Atualiza dinamicamente as ordens do painel Admin
        ...
        if (data.pos) {
            ...
            if (dist > 3.0) { // Se o GM teleportar ele, muda tudo
...
db.ref('cenario_inimigos').on('child_removed', snap => {
    ...
    if (estadoInimigos[id]) {
        ...
        console.log(`[Servidor] Inimigo ${id} apagado.`);
    }
});
...
setInterval(() => {
    ...
    for (let idInimigo in estadoInimigos) {
        ...
        if (alvosValidos.length > 0) {
            ...
            // Se o alvo está dentro do Campo de Visão
            if (dist <= estado.range) {
                viuAlguem = true;
                estado.ultimoAvistamento = agora; // Zera o relógio de perseguição
